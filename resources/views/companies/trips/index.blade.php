            ...
            <tbody>
                @foreach ($trips as $trip)
                    <tr>
                        ...
                        <td>{{ $trip -> duration }}</td>
                        <td>{{ $trip -> fare }}</td>
                        <td>{{ $ts = $trip -> number_of_seats }}</td>
                        <td>{{ $ts - $trip -> bookings -> count() }}</td>
                        <td>
                                ...
                                @elseif(!$trip->bookings->count())
                                    @if ($trip -> departure_date -> lt(Carbon\Carbon::today()->startOfDay()))
                                        {!! Form::open(['method' => 'DELETE', 'action'=>['CompaniesTripsController@destroy', $trip]]) !!}
                                            <button type="submit" class="third btn btn-xs btn-warning btn-block btn-xs">remove</button>
                                        {!! Form::close() !!}
                                    @elseif($trip -> departure_date -> gt(Carbon\Carbon::today()->endOfDay()))
                                        {!! Form::open(['method' => 'DELETE', 'action'=>['CompaniesTripsController@destroy', $trip]]) !!}
                                            <button type="submit" class="third btn btn-xs btn-warning btn-block btn-xs">delete</button>
                                        {!! Form::close() !!}
                                    @elseif($trip -> departure_date -> gt(Carbon\Carbon::today()->StartOfDay()) AND $trip -> departure_date -> tl(Carbon\Carbon::today()->endOfDay()))
                                        {!! Form::open(['method' => 'DELETE', 'action'=>['CompaniesTripsController@destroy', $trip]]) !!}
                                            <button type="submit" class="third btn btn-xs btn-warning btn-block btn-xs">remove</button>
                                        {!! Form::close() !!}
                                    @endif
